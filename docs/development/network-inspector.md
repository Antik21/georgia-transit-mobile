...
- Android sends Chucker only redacted, bounded inspection copies before any
  ...
  their preview is redacted before it becomes Chucker's response body. Supported
  unknown-length responses use the same capped non-consuming peek; their stored
  preview remains bounded and redacted. Binary, XML, declared-oversized, and
  unreadable responses become a small omission marker without exposing raw
  bytes to Chucker. iOS shows supported text/JSON/form bodies and omits binary,
  XML, streaming, and other unsupported bodies. Oversized supported bodies are
  marked truncated.
