...
seed_flow="$script_dir/flows/offline-route-cache-seed.yaml"
reopen_flow="$script_dir/flows/offline-route-cache-reopen.yaml"
fixture_port=8080
fixture_pid=
fixture_log=
fixture_health=
...
fixture_log=$(mktemp "${TMPDIR:-/tmp}/georgia-transit-offline-route-cache-log.XXXXXX")
fixture_health=$(mktemp "${TMPDIR:-/tmp}/georgia-transit-offline-route-cache-health.XXXXXX")

if [[ ! -f $fixture_path || ! -f $seed_flow || ! -f $reopen_flow ]]; then
  echo "Missing offline-route-cache fixture or flow" >&2
  exit 66
fi
...
maestro --device "$target" test "$seed_flow"
verify_owned_fixture || { echo "Fixture ownership or health changed during cache seeding" >&2; exit 70; }

fixture_is_our_process || { echo "Fixture PID ownership changed before shutdown; refusing to signal it" >&2; exit 70; }
kill "$fixture_pid"
wait "$fixture_pid" 2>/dev/null || true
fixture_pid=
if curl --fail --silent --max-time 1 "http://127.0.0.1:$fixture_port/healthz" >/dev/null 2>&1; then
  echo "Port $fixture_port is still serving after the owned fixture stopped; refusing a false offline run" >&2
  exit 70
fi

maestro --device "$target" test "$reopen_flow"
