...
private fun Content(state: ViewState, onAction: (Action) -> Unit) {
    Column(modifier = Modifier.fillMaxSize().testTag(AutomationId.MapScreen)) {
        ...
        MapPreview(modifier = Modifier.fillMaxWidth().weight(1f))
        Column(
...
private fun MapPreview(modifier: Modifier = Modifier) {
    Box(modifier = modifier.background(TransitColors.MapLand), contentAlignment = Alignment.Center) {
        Canvas(Modifier.fillMaxSize()) {
            drawRect(TransitColors.MapWater, topLeft = Offset(size.width * .72f, 0f), size = size.copy(width = size.width * .28f))
            drawLine(Color.White, Offset(0f, size.height * .25f), Offset(size.width, size.height * .72f), 18f, StrokeCap.Round)
            drawLine(Color.White, Offset(size.width * .08f, size.height), Offset(size.width * .62f, 0f), 12f, StrokeCap.Round)
            drawLine(TransitColors.RouteBlue, Offset(size.width * .1f, size.height * .72f), Offset(size.width * .86f, size.height * .3f), 8f, StrokeCap.Round)
            drawCircle(TransitColors.Accent, 15f, Offset(size.width * .42f, size.height * .54f))
            drawCircle(TransitColors.BrandDark, 15f, Offset(size.width * .68f, size.height * .4f))
        }
...
@Composable
private fun Preview() {
    GeorgiaTransitTheme { Content(ViewState("Tbilisi", listOf("301", "337")), onAction = {}) }
}
