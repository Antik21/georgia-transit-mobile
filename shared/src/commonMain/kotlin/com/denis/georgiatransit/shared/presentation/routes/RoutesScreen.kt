...
private fun Content(state: ViewState, onAction: (Action) -> Unit) {
    Column(
        ...
        if (state.routes.isEmpty()) {
            Text(stringResource(Res.string.routes_empty), modifier = Modifier.weight(1f))
        } else {
            LazyColumn(
...
private fun RouteRow(route: RouteItemUiModel, selected: Boolean, onClick: () -> Unit) {
    Card(modifier = Modifier.fillMaxWidth().testTag("routes.${route.shortName}").clickable(onClick = onClick)) {
        Row(
            ...
            Box(Modifier.size(36.dp).background(Color(route.colorArgb), CircleShape), contentAlignment = Alignment.Center) {
